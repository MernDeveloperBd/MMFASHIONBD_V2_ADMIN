...
useEffect(() => {
    ...
    fetchDataFromApi(`/api/category/${id}`).then((res) => {
        if (res?.category) {
            ...
            setPriviews(res.category.images || []);
        }
    });
    ...
    const handleSubmit = (e) => {
        ...
        editData(`/api/category/${isOpenFullScreenPanel?.id}`, formFields).then((res) => {                     
            setTimeout(() => {
                setIsLoading(false)
                ...
                <div className="scroll max-h-[72vh] overflow-y-scroll pr-4 pt-4 pb-4">
                    <div className="grid grid-cols-1 mb-3">
                        <div className="col w-[25%]">
                            ...
                            <input type="text" className="w-full h-[40px] border border-[rgba(0,0,0,0.1)] focus:outline-none focus:border-[rgba(0,0,0,0.4)] p-3 rounded-sm text-sm " defaultValue={formFields.name}  name='name'
