...
const AddColor = () => {
  ...
  const getData = async () => {
    const res = await fetchDataFromApi(`/api/product/productColor/get`);
    if (res?.error === false) {
      setData(res?.data);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) {
      openAlertBox('error', 'Please enter product Color');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!editId) {
        // 👉 Create
        res = await postData(`/api/product/productColor/create`, { name });
      } else {
        // 👉 Update
        res = await editData(`/api/product/productColor/${editId}`, { name });
        getData();
      }
  ...
  const handleDelete = async (id) => {
    await deleteData(`/api/product/productColor/${id}`);
    getData();
    openAlertBox('success', 'Product Color deleted');
  };

  const handleEdit = async (id) => {
    const res = await fetchDataFromApi(`/api/product/productColor/${id}`);
    if (res?.error === false) {
  ...
  return (
    <>
      <div className="col1 flex items-center justify-between px-2 py-0 ">
        <h3 className="text-[20px] font-[700]">Add Product Colors</h3>
      </div>
      ...
      <div className="card my-4 pt-5 pb-5 shadow-md sm:rounded-lg bg-white md:w-[65%]">
        <form className="form py-3 p-8 " onSubmit={handleSubmit}>
          <div className="col mb-4">
            <h3 className="text-[14px] font-[500] mb-1">Product Colors</h3>
            <input
              type="text"
